...
declare const Module: {
  cwrap: (name: 'gambatte_setinputgetter', returnType: string, argTypes: string[]) => (arg0: number, arg1: number, arg2: number) => undefined;
...
export const useControls = (gbPointer?: number) => {
  const [gambatteInputGetter, setGambatteInputGetter] = useState<((arg0: number, arg1: number, arg2: number) => undefined) | null>(null);
  const [buttonsFunctionPointer, setButtonsFunctionPointer] = useState<number | null>(null);
  ...
  useEffect(() => {
    setGambatteInputGetter(() => Module.cwrap('gambatte_setinputgetter', 'number', ['number', 'number', 'number']));
    setButtonsFunctionPointer(Module.addFunction(() => buttons.current, 'ii'));
  }, []);

  useEffect(() => {
    if (gbPointer === undefined || buttonsFunctionPointer === null || gambatteInputGetter === null) {
      return;
    }
